...
def recommend_superpixels(TEST_REGION, entropy, probability, cod, transformation_agg, superpixel_agg, student_id, al_cycle, updated_labels=None, use_forest=1):
    ...
    if config.COD: # A(original) + B(variance) + C(cod)
        ...
        if config.PROBABILITY:
            ...
            pred_offset_agg = pred_orig_offset - config.LAMBDA_2 * cod_loss_unpadded # for PROB: 0 means uncertain; for COD: 1 means uncertain # A+B+C
            ...
            entropy_agg = -entropy_orig - config.LAMBDA_2_A2 * cod_loss_unpadded # for ENT: 1 means uncertain; for COD: 1 means uncertain # A+B+C
    ...
    uncertainty_map = get_uncertainty_map(superpixels_group, superpixel_scores)
    np.save(f"./users/{student_id}/output/Region_{TEST_REGION}_uncertainty_C{al_cycle}.npy", uncertainty_map)
    ...
    plt.imsave(f'./users/{student_id}/output/R{TEST_REGION}_superpixels_C{al_cycle}.png', result_array)
...
def get_uncertainty_map(superpixels_group, superpixel_scores):
    uncertainty_map = np.zeros((config.HEIGHT, config.WIDTH))

    for sid, pixels in superpixels_group.items():
        score = superpixel_scores[sid]
        for (row, col) in pixels:
            uncertainty_map[row][col] = score
